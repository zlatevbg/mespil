...
@section('title', __('Evaristo real estate investment | Property investment in Algarve'))
@section('description', __('Invest in Evaristo, Algarve, Portugal. Real estate investment opportunity for joint venture partner to co-invest in a fully secured, off market property.'))

@section('content')
    {{-- <div id="property-video">
        <div class="container overlay-wrapper overlay-property">
            <div class="property-logo">
                <img loading="eager" src="{{ Helper::autover('/svg/{subdomain}/logo-evaristo-algarve-portugal-inverted.svg') }}" alt="{{ __('Logo: Evaristo Project, Algarve, Portugal') }}" height="124" width="200">
            </div>
        </div>
        <div class="embed-responsive embed-responsive-16by9">
            <video class="embed-responsive-item" autoplay muted playsinline preload="metadata" poster="{{ Helper::autover('/img/{subdomain}/investment-projects/evaristo-algarve-portugal/poster.jpg') }}">
                <source src="/video/evaristo-algarve-portugal.mp4" type="video/mp4"/>
            </video>
        </div>
    </div> --}}
    <div class="property-hero">
        <div class="embed-responsive embed-responsive-16by9 h-100">
            <img loading="eager" class="embed-responsive-item" src="{{ Helper::autover('/img/{subdomain}/investment-projects/evaristo-algarve-portugal/hero.jpg') }}" alt="{{ __('Facade of Evaristo Project, Algarve, Portugal') }}" height="1080" width="1920">
        </div>
        <div class="container overlay-wrapper overlay-property">
            <div class="property-logo">
                <svg class="logo-project-big" viewBox="0 0 100 25.96">
    ...
    <div class="container container-md-fluid space-single-top">
        <div class="d-flex flex-wrap justify-content-center align-items-center">
            <a class="prev-next-font internal-link d-flex flex-fill justify-content-start align-items-center mr-3" href="{{ Helper::route('investment-project', 'no-37-lisbon-portugal') }}">@fas('long-arrow-alt-left', 'mr-3 fa-2x')<span class="text-nowrap">{{ __('Previous project') }}</span><span class="text-nowrap prev-next-hidden">: {{ __('No.37 Lisbon, Portugal') }}</span></a>
            <a class="prev-next-font internal-link d-flex flex-fill justify-content-end align-items-center ml-3 text-right" href="{{ Helper::route('investment-project', 'no-9-faro-portugal') }}"><span class="text-nowrap">{{ __('Next project') }}</span><span class="text-nowrap prev-next-hidden">: {{ __('No.9 Faro, Portugal') }}</span>@fas('long-arrow-alt-right', 'ml-3 fa-2x')</a>
        </div>
    </div>
    ...
    <div class="container space-double space-single-top">
        <div class="row row-cols-1 row-cols-xl-2 mb-n5">
            <div class="col mb-5 position-relative">
                <h1 class="title-border title-border-orange mb-5">{{ __('Evaristo Project') }}<br><small class="h3 font-weight-normal">{{ __('Property investment in Algarve, Portugal') }}</small></h1>
                <p class="project-status status-orange text-white p-2">{{ __('Funding Required') }}</p>
                <p>{{ __('Joint venture partner required to co-invest in an excellent re-development project in a quaint upmarket area of Central-Algarve. This gated condominium in beautiful Evaristo is in walking distance to the unspoilt Praia do Castelo beach known for its crystal clear water and offers 56 x 1-bedroom apartments all with sea view.') }}</p>
                <div class="bg-light py-3">
                    <ul class="m-0 pr-4">
                        <li>{{ __('Gated & Privately Managed Condominium') }}</li>
                        <li>{{ __('Spacious 1 Bed Studios (avg. size 70m2)') }}</li>
                        <li>{{ __('95% Finished - 3 month Refurbishment Program') }}</li>
                        <li>{{ __('High demand for holiday rentals due to sea views, proximity to beach and several popular tourist villages') }}</li>
                        <li>{{ __('3 mins walk to the new W Hotel, Algarve') }}</li>
                    </ul>
                </div>
            </div>
            <div class="col mb-5">
                    ...
                    <table class="table table-hover mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="align-middle text-center">{{ __('Senior Debt') }}</th>
                                <th class="align-middle text-center">{{ __('Equity') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <th class="align-middle text-nowrap">{{ __('Investment') }}</th>
                                <td class="text-primary font-weight-bold h3 align-middle w-50 text-center text-nowrap">{{ __('€ 4 500 000') }}</td>
                                <td class="text-primary font-weight-bold h3 align-middle w-50 text-center text-nowrap">{{ __('€ 1 600 000') }}</td>
                            </tr>
                            <tr>
                                <th class="align-middle text-nowrap">{{ __('Term') }}</th>
                                <td class="text-center align-middle">{{ __('18 months with call option after 12 months') }}</td>
                                <td class="text-center align-middle">{{ __('6 months bridge finance') }}</td>
                            </tr>
                            <tr>
                                <th class="align-middle text-nowrap">{{ __('Security') }}</th>
                                <td class="text-center">{{ __('First lien') }}</td>
                                <td class="text-center">{{ __('Equity pledge SPV') }}</td>
                            </tr>
                            <tr>
                                <th class="align-middle text-nowrap">{{ __('Exit') }}</th>
                                <td class="text-center">{{ __('In line with sales of units') }}</td>
                                <td class="text-center">{{ __('On securing project finance') }}</td>
                            </tr>
                            <tr>
                                <th class="align-middle text-nowrap">{{ __('Sales Cycle') }}</th>
                                <td class="text-center" colspan="2">{{ __('Estimate 40% of units sold within 6 months') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="container space-single">
        <div class="row row-cols-1 row-cols-md-2 mb-n5 photoswipe-wrapper" data-photoswipe-gid="gallery">
            <a class="photoswipe col mb-5" href="{{ Helper::autover('/img/{subdomain}/investment-projects/evaristo-algarve-portugal/gallery/facade.jpg') }}" data-photoswipe-pid="facade" data-photoswipe-size="1440x960"><img loading="lazy" width="640" height="360" src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 640 360'%3E%3C/svg%3E" data-src="{{ Helper::autover('/img/{subdomain}/investment-projects/evaristo-algarve-portugal/gallery/thumbnails/facade.jpg') }}" class="img-fluid img-thumbnail" alt="{{ __('Facade of Evaristo Project, Algarve') }}"></a>
            <a class="photoswipe col mb-5" href="{{ Helper::autover('/img/{subdomain}/investment-projects/evaristo-algarve-portugal/gallery/evaristo.jpg') }}" data-photoswipe-pid="evaristo" data-photoswipe-size="2560x1920"><img loading="lazy" width="640" height="360" src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 640 360'%3E%3C/svg%3E" data-src="{{ Helper::autover('/img/{subdomain}/investment-projects/evaristo-algarve-portugal/gallery/thumbnails/evaristo.jpg') }}" class="img-fluid img-thumbnail" alt="{{ __('Evaristo Beach, Algarve') }}"></a>
        </div>
    </div>
    ...
    <div class="container space-single">
        <ol class="poi-wrapper list-unstyled mb-0">
            <li><a data-map-type="area" class="poi py-1" href="#project-map"><span class="fa-stack">@fas('circle', 'fa-stack-2x text-primary opacity-10') @far('circle', 'text-primary fa-stack-2x')</span><span>{{ __('Evaristo Project Location') }}</span></a></li>
...
@push('scripts')
    <script>
        ...
        let InfoWindow;
        const markers = [];
        const center = {lat: 37.078536, lng: -8.297017};
